...
| 항목 | 조건 | 배점 | 의미 |
|---|---|---|---|
| **Slope (기울기)** | 3시간 선형 회귀 기울기 $\ge 1.0\%$ | **+20** | 강력한 우상향 추세 |
| | $0.5\% \le$ 기울기 $< 1.0\%$ | **+10** | 완만한 상승 |
| | **음수 ($< 0$)** | **진입 불가** | 하락 추세는 절대 매수하지 않음 |
| **Channel Position** | 현재가가 3시간 채널(High-Low) 하단 30% 이내 | **+15** | 최적의 눌림목 구간 (가격 메리트) |
| | 중단 30~60% 구간 | +5 | 무릎에서 매수 |
...
*   **강한 상승 ($Slope \ge 2.0$)**: 현재가 대비 **-0.3%** 가격에 주문. (매수 기회를 놓치지 않기 위함)
*   **완만한 상승 ($0.5 \le Slope < 2.0$)**: 현재가 대비 **-1.0%** 가격에 주문. (충분한 조정 기다림)
*   **횡보/약반등**: 현재가 대비 **-1.5%** 가격에 주문. (안전마진 확보)
