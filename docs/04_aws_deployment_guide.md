...
**방법 A: 기존 Key Pair 복사 (가장 간편)**
1.  현재 PC에 있는 `.pem` 파일을 USB나 암호화된 메일 등으로 다른 PC로 안전하게 복사합니다.
2.  **주의**: 프로젝트 폴더나 Git에 `.pem` 파일을 절대 올리지 마세요. (해킹 위험)
3.  다른 PC에서도 동일하게 SSH 접속 명령어를 사용합니다.

**방법 B: 새로운 SSH Key 등록 (보안 권장)**
1.  새로운 PC에서 터미널을 열고 키 생성: `ssh-keygen -t rsa -b 4096` (엔터 계속 입력)
2.  생성된 공개키(`~/.ssh/id_rsa.pub`) 내용을 복사합니다.
3.  기존 PC로 서버에 접속한 뒤, `~/.ssh/authorized_keys` 파일 밑에 붙여넣고 저장합니다.
4.  이제 새 PC에서는 `.pem` 파일 없이도 바로 접속 가능합니다.
